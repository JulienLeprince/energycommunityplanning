...
for sa_setup in sa_setups:
        ...
        for sa in SA_scenarios:
            ...
            E_blg_in = pulp.LpVariable.dicts('var_E_blg_in', (range(scenarios), buildings, range(H + 1)), lowBound=0, upBound=E_lv_max, cat='Continous')
            E_blg_out = pulp.LpVariable.dicts('var_E_blg_out', (range(scenarios), buildings, range(H + 1)), lowBound=0, upBound=E_lv_max, cat='Continous')
            ...
            for s in range(scenarios):
                ...
                for b in buildings:
                    ...
                    for t in range(H):
                        ...
                        my_lp_problem += dfb[s_occ][b]['E_blg'].iloc[t] + E_blg_bat_ch[s][b][t] + E_blg_hp[s][b][t] + E_blg_out[s][b][t] \
                    ...
                    my_lp_problem += E_blg_bat[s][b][0] == 0
                    my_lp_problem += Q_tes[s][b][0] == 0
                for t in range(H):
                ...
                # Initial conditions
                my_lp_problem += E_com_bat[s][0] == 0
                my_lp_problem += E_com_hyd[s][0] == 0
                # Sizing
