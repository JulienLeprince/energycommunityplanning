...
arg_nb_of_iterations = int(10)  # number of iterations for the distributed optimization problem to converge
arg_folder_name = 'demo_folder'

sa_setup = sys.argv[7]   # ['userbehavior', 'climate', 'economic']
sa = int(sys.argv[8])
...
if sa_setup == 'userbehavior':
    s_clim = s_climate_and_eco
    s_eco = s_climate_and_eco
    s_occ = sa
    ...
    s_clim = s_occ_and_climate
    s_occ = s_occ_and_climate
    s_eco = sa
...
iterations = range(arg_nb_of_iterations)
...
blg_t_cols = ['T_blg', 'O_slk_blg', 'E_blg_bat', 'Q_tes', 'Q_stc', 'Q_sp', 'Q_tes_ch', 'Q_tes_dch', 'Q_hp',
    ...
    for b in buildings:
        ...
        for s in range(scenarios):
            ...
            for t in range(H):
                ...
                my_lp_problem += T_blg[s][t] >= dfb[s_occ][b]['T_blg_set'].iloc[t] - T_blg_buffer  # heating boundary
                # Battery
                ...
                my_lp_problem += Q_stc[s][t] == A_blg_stc[s] * eff_blg_stc * (dfw[s_clim]['Q_sol'].iloc[t]
                                                                                    - U_blg_stc * (T_stc - dfw[s_clim]['T_a'].iloc[t]))
                # Energy balance
                my_lp_problem += Q_sp[s][t] + Q_tes_ch[s][t] == Q_tes_dch[s][t] + Q_hp[s][t] + Q_bol[s][t]
                my_lp_problem += dfb[s_occ][b]['E_blg'].iloc[t] + E_blg_bat_ch[s][t] + E_blg_hp[s][t] + E_blg_out[s][t] \
            ...
            # Initial conditions
            my_lp_problem += T_blg[s][0] == dfb[s_occ][b]['T_blg_set'].iloc[0]
            ...
            my_lp_problem += O_opr[s] == pulp.lpSum(E_hv_in[s][t] * p_elec[s_eco][t] for t in range(H)) \
                                + pulp.lpSum(V_blg_gas[s][t] * p_gas[s_eco][t] for t in range(H))
        ...
        for s in range(scenarios):
            ...
            expected_costs = np.sum(O_buildings_per_scenario)*probabilities.loc[s]
...
file = path_out + 'SA_distrib' + version + '_nbblgs' + str(len(buildings)) + '_setup_' + sa_setup + str(sa)
